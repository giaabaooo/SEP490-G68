        ...
        <div className="lg:col-span-2 bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden flex flex-col h-fit">
                      ...
                      <tr key={job._id || job.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="p-5 text-center">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white rounded-xl font-bold text-xs transition-all shadow-2xs"
                        ...
                        <td className="p-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-xl transition-colors ${jobStatus === 'active' ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'
                                }`}
                            >
          ...
          {jobs.length > 5 && (
            <div className="p-4 bg-slate-50/70 border-t border-slate-100 text-center">
              ...
              >
                Xem tất cả chiến dịch ({jobs.length}) <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          )}
        </div>

        <div className="bg-white rounded-[32px] p-7 shadow-sm border border-slate-200 mb-12 flex flex-col justify-between">
          <div>
            ...
            {statsData.trend && statsData.trend.length === 0 ? (
              ...
              <div className="space-y-4">
                {(statsData.trend || []).slice(0, 5).map((t, idx) => (
          ...
          {(statsData.trend?.length || 0) > 5 && (
            <div className="pt-5 mt-4 border-t border-slate-100 text-center">
              ...
              >
                Xem tất cả lịch sử ({statsData.trend.length} ngày) <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          )}
        </div>
