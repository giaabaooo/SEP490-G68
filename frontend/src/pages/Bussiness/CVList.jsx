...
import { Search, Filter, Eye, CheckCircle, XCircle, Download, Sparkles, Clock, ArrowLeft, DownloadCloud, X, ThumbsUp, AlertTriangle, RefreshCw, Layers, Mail, UserCheck } from 'lucide-react';
...
const CVList = () => {
  ...
  const getDetailedStatusBadge = (app) => {
    if (app.status === 'Testing') {
      if (app.testStatus === 'Completed') {
        return {
          label: 'Đã hoàn thành Test',
          style: 'bg-purple-50 text-purple-700 border-purple-200'
        };
      }
      if (app.testStatus === 'In_Progress') {
        return {
          label: 'Đang làm bài Test',
          style: 'bg-amber-50 text-amber-700 border-amber-200'
        };
      }
      return {
        label: 'Chờ làm bài Test',
                    ...
                    columnApps.map((app) => (
                      <div key={app._id || app.id} draggable onDragStart={(e) => handleDragStart(e, app._id || app.id)} className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-400 transition-all cursor-grab active:cursor-grabbing group relative overflow-hidden">
                        <div className="flex items-start gap-3 mb-3">
                          <img src={app.userId?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(app.userId?.fullName || 'U')}&background=eff6ff&color=3b82f6`} alt={app.userId?.fullName} className="w-10 h-10 rounded-full border border-slate-100 object-cover shrink-0" />
                          <div className="flex-grow min-w-0">
                            <button onClick={() => navigate(`/bussiness/candidate/${app._id || app.id}`)} className="font-bold text-black text-sm hover:text-blue-600 transition-colors text-left block w-full mb-0.5 truncate">{app.userId?.fullName || 'Unknown'}</button>
                            <p className="text-black text-[10px] font-bold uppercase tracking-wider truncate">{app.jobId?.title || '—'}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-1.5 flex-wrap">
                          <button onClick={() => handleOpenAiModal(app)} title="Xem báo cáo chi tiết AI CV" className={`px-2 py-1 rounded-lg border flex items-center gap-1 text-[11px] font-bold transition-all ${getAiScoreStyle(app.aiScore ?? 0)}`}>
                            {(app.aiScore ?? 0) >= 80 && <Sparkles className="w-3.5 h-3.5" />}
                            <span>AI CV: {(app.aiScore ?? 0)}%</span>
                          </button>

                          {app.testStatus === 'Completed' ? (
                            <span title="Điểm bài kiểm tra chuyên môn" className="px-2 py-0.5 rounded-lg border text-[11px] font-black bg-purple-50 text-purple-700 border-purple-200">
                              Test: {app.testScore ?? 0}/100
                            </span>
                          ) : (app.hasTest || app.jobId?.requireTest || app.assessmentId) ? (
                            <span className="px-1.5 py-0.5 rounded-lg border text-[10px] font-medium bg-slate-50 text-slate-500 border-slate-200">
                              {app.testStatus === 'In_Progress' ? 'Đang test' : 'Chưa test'}
                            </span>
                          ) : null}
                        </div>
