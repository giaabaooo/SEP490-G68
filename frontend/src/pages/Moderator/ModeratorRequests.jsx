        ...
        {loading ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {requests.length === 0 && (
                <tr>
                  <td colSpan="5" className="p-10 text-center text-slate-500 font-medium">Hiện tại chưa có yêu cầu tạo Test nào.</td>
                </tr>
              )}
              ...
              {requests.map((req) => (
                <tr key={req.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-5">
                    {req.status === 'pending' ? (
                      <span className="px-3 py-1.5 bg-amber-100 text-amber-700 rounded-lg text-[11px] font-black flex items-center w-fit">
                        <Clock className="w-3 h-3 mr-1.5" /> Chờ tạo Test
                      </span>
                    ) : (
                      <span className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg text-[11px] font-black flex items-center w-fit">
                        <CheckCircle className="w-3 h-3 mr-1.5" /> Đã duyệt
                      </span>
                    )}
                  </td>
                  <td className="p-5">
                    <div className="flex justify-center gap-2">
                      ...
                      <button
                        ...
                        className="p-2 text-slate-400 hover:text-emerald-600 bg-slate-100 hover:bg-emerald-50 rounded-lg transition-colors shadow-sm"
                      ...
                      {req.status === 'pending' && (
                        <button
                          ...
                          className="px-3 py-2 text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg flex items-center gap-1.5 text-xs font-bold transition-transform hover:-translate-y-0.5 shadow-sm"
                        >
