            ...
            <div className="max-w-6xl mx-auto px-4 -mt-10 relative z-20">
                <div className="flex justify-between items-center mb-6 px-2">
                    <p className="font-medium text-slate-600">
                        Tìm thấy <span className="font-black text-blue-600 bg-white px-2 py-0.5 rounded-md shadow-sm">{filteredTests.length}</span> chủ đề
                    </p>
                </div>
                            ...
                            return (
                                <div key={test._id} className={`bg-white rounded-3xl p-6 border flex flex-col hover:-translate-y-1 hover:shadow-xl transition-all duration-300 ${isPaid ? 'border-amber-200 hover:border-amber-400 shadow-amber-500/5' : 'border-slate-200 hover:border-blue-300'}`}>
                                    ...
                                    <div className="pt-5 mt-5 border-t border-slate-100">
                                        {/* Bạn có thể gắn thanh toán ở đây, nếu isPaid nhưng chưa thanh toán -> hiển thị Mua Ngay. Ở đây tạm thời gán link mở đề luôn */}
                                        <button
                                            onClick={() => {
                                                if(isPaid) {
