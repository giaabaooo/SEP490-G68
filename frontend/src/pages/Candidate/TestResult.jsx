    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 p-4 animate-fadeIn">
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center relative overflow-hidden shadow-2xl">
                ...
                <button onClick={onClose} className="absolute top-4 right-4 text-black hover:text-black cursor-pointer"><X className="w-5 h-5"/></button>
                ...
                <p className="text-sm font-medium text-black mb-6 leading-relaxed">{message}</p>
...
export default function TestResult() {
    ...
    const handleGenerateRoadmap = async () => {
        ...
        if ((isPractice || goal !== 'Vượt qua phỏng vấn kỹ thuật') && timeframe === 'custom') {
            ...
            finalTimeframe = calculatedDeadlineText.split(' (')[0]; 
        }
            ...
            <div className="max-w-4xl mx-auto px-4 mt-8 space-y-6">
                <div className="flex justify-center mb-6">
                    <div className="bg-white p-1.5 rounded-2xl flex gap-2 border border-slate-200 shadow-sm">
                        <button onClick={() => setActiveTab('RESULT')} className={`px-8 py-3 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'RESULT' ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-black hover:bg-slate-50 hover:text-black'}`}>
                            <CheckCircle2 className="w-4 h-4" /> Chi tiết Điểm số
                        </button>
                        <button onClick={() => setActiveTab('ROADMAP')} className={`px-8 py-3 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'ROADMAP' ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-black hover:bg-slate-50 hover:text-black'}`}>
                ...
                {activeTab === 'RESULT' && (
                    <div className="animate-fade-in space-y-6">
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-around gap-8">
                            <div className="text-center">
                                <p className="text-xs font-bold text-black uppercase tracking-widest mb-2">ĐIỂM SỐ ĐẠT ĐƯỢC</p>
                                <p className={`text-6xl font-black ${app.testScore >= 50 ? 'text-emerald-500' : 'text-rose-500'}`}>{app.testScore}<span className="text-2xl text-black">/100</span></p>
                            </div>
                            <div className="hidden md:block w-px h-20 bg-slate-100"></div>
                            <div className="flex gap-12 text-center">
                                <div>
                                    <p className="text-xs font-bold text-black uppercase tracking-widest mb-2">THỜI GIAN LÀM</p>
                                    <p className="text-2xl font-black text-black flex items-center justify-center gap-2"><Clock className={`w-5 h-5 ${isPractice ? 'text-amber-500' : 'text-indigo-500'}`} /> {formatDuration(app.testDuration)}</p>
                                </div>
                                <div>
                                    <p className="text-xs font-bold text-black uppercase tracking-widest mb-2">SỐ CÂU ĐÚNG</p>
                                    <p className="text-2xl font-black text-black">{Object.keys(answers).filter(key => test?.questions[key] && answers[key] === test.questions[key].correctAnswer).length} / {test?.questions?.length || 0}</p>
                                </div>
                            </div>
                        </div>

                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                            <h3 className="text-xl font-black text-black mb-8 border-b border-slate-100 pb-4">Chi tiết đáp án</h3>
                                    ...
                                    return (
                                        <div key={idx} className={`border-l-4 rounded-2xl p-6 bg-slate-50 border border-slate-200 ${isCorrect ? 'border-l-emerald-500' : 'border-l-rose-500'}`}>
                                            <div className="flex justify-between items-start mb-4">
                                                <h4 className="text-base font-bold text-black pr-4">Câu {idx + 1}: {questionText}</h4>
                                            ...
                                            <div className="space-y-2">
                                                {q.options.map((opt, optIdx) => {
                                                    ...
                                                    let style = "bg-white border-slate-200 text-black";
                                                    let Icon = null;
                    ...
                    <div className="animate-fade-in space-y-6">
                        {isFetchingRoadmap ? (
                            <div className="bg-white rounded-3xl p-16 shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-black font-bold">Đang tải dữ liệu lộ trình...</p>
                            </div>
                        ) : !roadmap ? (
                            <div className="bg-white rounded-3xl p-16 shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center">
                            ...
                            <>
                                ...
                                <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                                    <h3 className="text-xl font-black text-black mb-6 flex items-center gap-2"><Map className="w-6 h-6 text-indigo-500" /> Các Giai Đoạn Học Tập</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {roadmap.content.weeks?.map((w, idx) => (
                                            <div key={idx} className="bg-slate-50 border border-slate-100 rounded-2xl p-6 relative overflow-hidden group">
                                                ...
                                                <h5 className="text-lg font-bold text-black mb-4 line-clamp-2">{w.focus}</h5>
                                                <ul className="space-y-2">
                                                    {w.tasks?.map((task, taskIdx) => (
                                                        <li key={taskIdx} className="flex items-start gap-2 text-sm font-medium text-black">
                                ...
                                {roadmap.content.suggestedCourses?.length > 0 && (
                                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                                        <h3 className="text-xl font-black text-black mb-6 flex items-center gap-2"><BookOpen className="w-6 h-6 text-emerald-500" /> Đề xuất Khóa Học</h3>
                                            ...
                                            {roadmap.content.suggestedCourses.map((course, idx) => (
                                                <div key={idx} className="border border-slate-200 rounded-2xl p-5 hover:border-emerald-300 hover:shadow-lg transition-all flex flex-col h-full bg-white">
                                                    <div className="flex justify-between items-start mb-3">
                                                        ...
                                                        {course.isFree && <span className="text-xs font-bold text-black">FREE</span>}
                                                    </div>
                                                    <h4 className="font-bold text-black mb-2 leading-snug line-clamp-2">{course.title}</h4>
                                                    <p className="text-xs font-medium text-black mb-5 flex-1 line-clamp-3 leading-relaxed">{course.reason}</p>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[32px] w-full max-w-[450px] shadow-2xl overflow-hidden animate-scale-in">
                        ...
                        <div className="p-6 space-y-6">
                            <div>
                                <label className="block text-xs font-black text-black uppercase tracking-widest mb-2">Mục tiêu cốt lõi</label>
                                <select 
                                    value={goal} 
                                    onChange={e => setGoal(e.target.value)} 
                                    className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 font-bold text-black bg-slate-50 cursor-pointer"
                                >
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {timeframe === 'custom' && (
                                            <input 
                                                ...
                                                className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 font-medium text-sm text-black"
                                                autoFocus
                                            />
                                        )}
                                    </div>
