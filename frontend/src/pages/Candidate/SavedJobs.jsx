    ...
    <div className="bg-slate-50 min-h-screen pb-12 font-inter">
      <div className="bg-white border-b border-slate-200 pt-16 pb-16 px-4">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-black text-base font-medium">Danh sách công việc bạn đã lưu để xem lại sau.</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-black font-medium">Đang tải danh sách đã lưu...</p>
          </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-5">
                    <Link to={`/jobs/${jobId}`} className="flex-1 flex flex-col md:flex-row gap-5">
                      ...
                      <div className="flex-1">
                        <h2 className="text-lg font-bold text-black group-hover:text-emerald-600 transition-colors line-clamp-1">{job.title}</h2>
                        <p className="text-sm font-semibold text-black mt-1">{job.companyName}</p>

                        <div className="flex flex-wrap items-center gap-4 mt-3 text-sm font-medium text-black">
                          <div className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-black" /> {job.location || 'Chưa cập nhật'}</div>
                          <div className="flex items-center gap-1.5 text-emerald-600"><DollarSign className="w-4 h-4" /> {job.salary || 'Thỏa thuận'}</div>
                          <div className="flex items-center gap-1.5"><Briefcase className="w-4 h-4 text-black" /> {job.experience}</div>
                        </div>

                        <div className="flex flex-wrap items-center gap-2 mt-4">
                          <span className="px-3 py-1 bg-slate-100 text-black text-xs font-bold rounded-md">{job.type}</span>
                    ...
                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start mt-4 md:mt-0 border-t md:border-t-0 pt-4 md:pt-0 border-slate-100">
                      ...
                      <div className="flex items-center gap-1 text-xs font-medium text-black mt-auto">
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 mt-10 mb-4">
                <button 
                  ...
                  className="px-4 py-2 rounded-xl border border-slate-200 text-black disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 font-bold text-sm transition-colors"
                >
                  Trước
                </button>
          ...
          <div className="bg-white rounded-2xl p-12 shadow-sm border border-slate-200 text-center">
            ...
            <h3 className="text-lg font-bold text-black mb-2">Chưa có việc làm nào được lưu</h3>
            <p className="text-black font-medium text-sm">Hãy quay lại trang việc làm để lưu những cơ hội phù hợp.</p>
